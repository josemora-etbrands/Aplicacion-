...
function buildBookmarklet(secret: string): string {
  ...
  // Lee todas las páginas de sales_speed y envía en lotes de 300 a /api/ingest-velocities
  return `javascript:(async()=>{const A=${JSON.stringify(APP_URL)},S=${s};try{const all=[];let p=1,T=1;do{const r=await fetch('/api/internal/sales_speed/product_items?page='+p+'&week_count=5&sort_key=weekly_sales_speed&sort_dir=desc&active=active&page_size=100',{headers:{Accept:'application/json'},credentials:'include'});if(r.url.indexOf('/session/new')>-1){alert('Tu sesión de ProfitGuard expiró. Inicia sesión y reintenta.');return;}const j=await r.json();T=(j.meta&&j.meta.total_pages)||1;(j.items||[]).forEach(function(i){all.push({sku:i.sku,weeklySalesSpeed:i.weeklySalesSpeed,category:i.category,averageWeeklySales:i.averageWeeklySales,totalStock:i.totalStock,associationsCount:i.associationsCount,weeks:(i.weeklySales||[]).map(function(w){return{number:w.number,year:w.year,units:w.units};})});});p++;}while(p<=T);let proc=0;for(let i=0;i<all.length;i+=300){const res=await fetch(A+'/api/ingest-velocities',{method:'POST',headers:{'Content-Type':'application/json','x-ingest-secret':S},body:JSON.stringify({items:all.slice(i,i+300)})});const o=await res.json();if(!res.ok){alert('Error: '+(o.error||res.status));return;}proc+=(o.stats&&o.stats.processed)||0;}alert('\\u2713 Velocidad de Ventas actualizada: '+proc+' productos');}catch(e){alert('Error: '+e);}})();`;
}
